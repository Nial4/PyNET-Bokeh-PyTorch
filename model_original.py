# model coding by Andrey Ignatov. recoding for Bokeh task by Haiya Huang
...
class PyNET(nn.Module):
    ...
    def __init__(self, level, instance_norm=True, instance_norm_level_1=False):
        ...
        self.conv_l1_d1 = ConvMultiBlock(16, 32, 3, instance_norm=False)  # 256 -> 256  L1 input
        self.pool1 = nn.MaxPool2d(2, 2)  # 256 -> 128

        self.conv_l2_d1 = ConvMultiBlock(32, 64, 3, instance_norm=instance_norm)  # 128 -> 128  L2 input
        self.pool2 = nn.MaxPool2d(2, 2)  # 128 -> 64

        self.conv_l3_d1 = ConvMultiBlock(64, 128, 3, instance_norm=instance_norm)  # 64 -> 64   L3 input
        self.pool3 = nn.MaxPool2d(2, 2)  # 64 -> 32

        self.conv_l4_d1 = ConvMultiBlock(128, 256, 3, instance_norm=instance_norm)  # 32 -> 32  L4 input
        ...
        self.conv_t4a = UpsampleConvLayer(512, 256, 3)  # 16 -> 32
        self.conv_t4b = UpsampleConvLayer(512, 256, 3)  # 16 -> 32

        self.conv_l5_out = ConvLayer(512, 3, kernel_size=3, stride=1, relu=False)
        self.output_l5 = nn.Tanh()
        ...
        self.conv_t1a = UpsampleConvLayer(64, 32, 3)  # 128 -> 256
        self.conv_t1b = UpsampleConvLayer(64, 32, 3)  # 128 -> 256

        self.conv_l2_out = ConvLayer(64, 3, kernel_size=3, stride=1, relu=False)
        self.output_l2 = nn.Tanh()

        # -------------------------------------L1 Processing: Level 1,  Input size: 256 x 256
        ...
        self.conv_t0 = UpsampleConvLayer(32, 16, 3)  # 256 -> 512
        self.output_l1 = nn.Tanh()

        # -------------------------------------L0

        self.conv_l0_out = ConvLayer(16, 3, kernel_size=3, stride=1, relu=False)
        self.conv_t2x = UpsampleConvLayer(3, 3, 3)  # 512 -> 1024
        self.output_l0 = nn.Tanh()

        # -------------------------------------2x
        self.conv_l2x_out = ConvLayer(3, 3, kernel_size=3, stride=1, relu=False)
        self.output_l2x = nn.Tanh()  # -> 1024
    ...
    def level_0(self, conv_t0):
        ...
        conv_l0_d1 = self.conv_l0_out(conv_t0)
        conv_t2x = self.conv_t2x(conv_l0_d1)
        output_l0 = self.output_l0(conv_l0_d1)

        return output_l0, conv_t2x

    def level_2x(self, conv_t2x):

        conv_l2x_d1 = self.conv_l2x_out(conv_t2x)
        output_l2x = self.output_l2x(conv_l2x_d1)

        return output_l2x

    def forward(self, x):

        input = space_to_depth(x, 2)

        conv_l1_d1 = self.conv_l1_d1(input)
        ...
        if self.level < 1:
            output_l0, conv_t2x = self.level_0(conv_t0)
        if self.level < 0:
            output_l2x = self.level_2x(conv_t2x)
